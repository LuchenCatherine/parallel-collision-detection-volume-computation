...
public:
  ...
  std::vector<DATATYPE> Search2(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::function<bool (const DATATYPE&)> callback) const;
...
protected:
  ...
  bool Search2(Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool (const DATATYPE&)> callback, std::vector<DATATYPE>& result) const;
...
RTREE_TEMPLATE
std::vector<DATATYPE> RTREE_QUAL::Search2(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::function<bool (const DATATYPE&)> callback) const
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
  ...
  int foundCount = 0;
  Search2(m_root, &rect, foundCount, callback, result);
...
// Search in an index tree or subtree for all data retangles that overlap the argument rectangle.
RTREE_TEMPLATE
bool RTREE_QUAL::Search2(Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool (const DATATYPE&)> callback, std::vector<DATATYPE>& result) const
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        if(!Search2(a_node->m_branch[index].m_child, a_rect, a_foundCount, callback, result))
        {
          // The callback indicated to stop searching
          return false;
        }
